...
from .compat import migrate_legacy_flat_apply_config, migrate_legacy_flat_evaluate_config
...
class ApplyOverrides(BaseModel):
    ...
    @classmethod
    def _migrate_legacy_flat_layout(cls, value):
        # Compatibility only: new configs must use the nested schema directly.
        return migrate_legacy_flat_apply_config(value)
...
class EvaluateOverrides(BaseModel):
    ...
    def _migrate_legacy_flat_layout(cls, value):
        # Compatibility only: new configs must use the nested schema directly.
        return migrate_legacy_flat_evaluate_config(value)
